...
\end{latexcode}
  ...
  \lstinline[style=latexcode]|\UnitCode|,
  \lstinline[style=latexcode]|\UnitName|,
  \lstinline[style=latexcode]|\UnitURL| and
  \lstinline[style=latexcode]|\QuizzesURL| respectively. If the
  command \lstinline[style=latexcode]|\QuizzesURL| is omitted then the URL for the quiz
  index file is set to \lstinline[style=latexcode]|\UnitURL/|Quizzes. (Within the School of
  ...
  \lstinline[style=latexcode]|\title| command. Note that the \lstinline[style=latexcode]|\title|
  command \emph{must} appear before the \lstinline[style=latexcode]|\begin{document}| command.
...
\end{latexcode}
  ...
  The correct answer goes after \lstinline[style=latexcode]|\correct|, incorrect answers
  after \lstinline[style=latexcode]|\incorrect|. Either can be followed by a
  \lstinline[style=latexcode]|\response|, providing comments on the students choice when
  checking their answers.  The \lstinline[style=latexcode]|\response| commands are optional;
...
\end{latexcode}
  ...
  \lstinline[style=latexcode]|\correct| commands may be used.
  \par
...
\begin{discussion}[Question 4]
  ...
  The answer is typeset using the \lstinline[style=latexcode]|\answer| macro. That macro
  ...
  syntax is \lstinline[style=latexcode]|\answer[text after answer box]{numerical answer}|.
  Then there is a mechanism for providing feedback for correct and
  incorrect answers. This is done using \lstinline[style=latexcode]|\whenRight| and
  \lstinline[style=latexcode]|\whenWrong|.  Unlike the \lstinline[style=latexcode]|\response| commands, the two
  responses \lstinline[style=latexcode]|\whenRight| and \lstinline[style=latexcode]|\whenWrong| are both mandatory
...
\lstinline[style=latexcode]|\quiz| then
