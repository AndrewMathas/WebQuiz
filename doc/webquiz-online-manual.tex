...
\end{latexcode}
  ...
  recommended that you use them because targetted feedback to ythe
  students based on their responses can be beneficial.
  \par
  For example, the code below, when run through \WebQuiz, produces \qref{1}
  in ghe on-line manual quiz.
\begin{latexcode}
   \begin{question}
...
\begin{discussion}[Question 3]
  ...
  used apringly as multiple columns do not alsays display well if the
...
\begin{discussion}[Question 4]
  ...
  macro takes two arguments: an optional comparision method, which
...
\begin{discussion}[Credits]
   ...
   Don's idea of using \TeX 4ht, and some of his code, is still in use.
