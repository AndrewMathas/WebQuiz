...
\begin{discussion}[Question 4]
  ...
  The answer is typeset using the \LatexCode|\answer| macro. That macro
  ...
  syntax is \LatexCode|\answer[text after answer box]{numerical answer}|.
  Then there is a mechanism for providing feedback for correct and
  incorrect answers. This is done using \LatexCode|\whenRight| and
  \LatexCode|\whenWrong|.  Unlike the \LatexCode|\response| commands, the two
  responses \LatexCode|\whenRight| and \LatexCode|\whenWrong| are both mandatory
  for questions of this type.  The syntax is for such questions is as
  follows:
\begin{latexcode}
       \begin{question}
         . . .question text. . .
         \answer[integer|number|string|lowercase|evaluate]{actual answer}
         \whenRight . . . feedback when right
         \whenWrong . . . feedback when wrong
       \end{question}
\end{latexcode}
  \par
  The optional argument to the \LatexCode{\answer} command sets the type
  of comparison that is used when marking the quiz. By default, integer
...
\begin{discussion}[Index Files]
  ...
  This is done with a \WebQuiz{} file which contains a \LatexCode|quizlist|
