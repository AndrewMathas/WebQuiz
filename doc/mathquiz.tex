...
\section{Introduction}
    ...
    teaching materials. In principle, a \MathQuiz quiz can contain anything
    ...
    The underlying engine used by \MathQuiz is \ctan{tikz} so, because
    ...
    \ctan[pgf]{tikz} and \ctan{pstricks} in \MathQuiz quizzes, although
...
\subsection{Credits}
 ...
 \section{The MathQuiz document class --- the \LaTeX{} commands}\label{S:documentclass}
  ...
  \mathquizopt{initialise}; see \autoref{S:configuration} for more
  details.
...
discussion.
  ...
  \noindent
  or by setting \textsf{breadcrumbs} in the \mathquizrc file equal to the string
  \begin{center}
  ...
  Similarly, setting \textsf{breadcrumbs = none} in the \mathquizrc will
...
\begin{description}
  \Item{BreadCrumb}
    ...
    variable, which by default is the last item in the breadcrumbs on
    ...
    first colon. For example, the title
     ...
     sets the breadcrumb to ``Quiz 1''. Using the \Verb|\Breadcrumb|
  ...
  \Item{Department}
    ...
    \texttt{mathquiz \textemdash\textemdash edit-settings}.
...
\LaTeX{} are \ctan{pstricks} and \ctan[pgf]{tikz}. Unfortunately, both of
...
Using \ctan{pstricks} is, in some sense, easier as it seems to work
better with \ctan{tex4ht}. There is a significant caveat, however,
because the conversion of \ctan{pstricks} images is done using
\ctan{dvisvgm} so it is necessary to make sure that this package is
correctly installed. At first sight this looks OK because \ctan{dvisvgm}
is included in \TeX Live and Mik\TeX, however, , \ctan{dvisvgm} uses
...
\begin{description}
  \item[pst2pdf] \index{pst2pdf}\index{pstricks}
    ...
    \ctan{pst2pdf} to a \MathQuiz file in order to convert all postscript objects in the
  ...
  \item[tikz]\index{tikz}
    ...
    \ScreenShot{Tikz example}{examples/tikz}
...
option.
  ...
  MathQuiz settings stored in /Users/andrew/.mathquizrc
    ...
    \subsection{The \mathquizrc file}\label{SS:customisation}\label{SS:rcfile}\index{default settings}
    ...
    \begin{description}
        ...
        \item[mathquiz\_www] This is the full path to the mathquiz web
            ...
            then the command
            \begin{center}
                    \texttt{cd \$(mathquiz \ddash settings mathquiz\_www)/docs}
            \end{center}
            will take you to the \MathQuiz on-line \texttt{docs/} directory.
        \item[breadcrumbs]
  ...
  \subsubsection{Changing the format of the \MathQuiz web pages}\index{format}
  ...
  produced by \MathQuiz. For example, by adding code to the
  ...
  change the CSS style sheet for mathquiz, which is the file
  \textsf{web/mathquiz.css}. More sophisticated versions of
...
\section{The on-line MathQuiz manual}\label{S:online}
  ...
  manual} that is a \LaTeX file written with the \textsf{mathquiz}
  document class. The conversion of the manual from \LaTeX{} to HTML is
  done by \MathQuiz. The PDF version of this manual is included here for
  your convenience. The source file for the on-line manual is included
  in the documentation of \MathQuiz to allow you to create a local
  version of the on-line manual. Look for the file
  \texttt{mathquiz-online-manual.tex} in the \textsf{mathquiz\_www/docs}
  directory; see~\autoref{SS:rcfile}.

  The on-line manual was written for ``internal use'' when \MathQuiz was
  first written in 2004. \MathQuiz has evolved quite a lot since then.
  ...
  features of \MathQuiz or how to use the program.  If there are any
  ...
  \Verb|latex mathquiz-online-manual| to create a \textsf{dvi file}. The
