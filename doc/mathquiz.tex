...
discussion.
  ...
  \begin{description}
    ...
    addition, a drop-down menu to the index page is added added to the
    breadrumbs, which is accessed via {\large\color{red} $\equiv$}.
...
\begin{description}
  ...
  \Item{DepartmentURL}
    ...
    the kilnk in the first breadcrumb on each web page.
  ...
  \Item{Institution}
    ...
    (provided that the screensize is not too small).  As described
...
\begin{description}
  \item[pst2pdf] \index{pst2pdf}\index{pstricks}
    ...
    \ctan{pst2pdf} to a \MathQuiz file in order to to convert all postscript objects in the
    ...
    \textit{25-09-2017: Recently \textsf{pst2pdf} has stopped working
    for me. It used to make the example above display correctly,
    however, it no longer does. I do not know whether the problem is
    occurs only on my computer or whether it is more general.}
