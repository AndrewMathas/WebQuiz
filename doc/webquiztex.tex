...
% load webquiztex-ini and webquiztex-doc code -- doc needs ini so ini is first
\input{webquiztex-ini.code}
\input{webquiztex-doc.code}
...
\NewDocumentCommand\OnlineManual{s}{%
  \href{http://www.maths.usyd.edu.au/u/mathas/WebQuizTeX/webquiztex-online-manual.html}{Online manual}%
  \IfBooleanF{#1}{ (\hyperref[S:online]{Appendix B})}\xspace%
}

\indexprologue{%
  \noindent\textit{This is an index only for the main \WebQuizTeX manual.
  It does not index the \OnlineManual.}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{webquiztex.themes} % defines a comma separated list of themes \WebQuizThemes
...
\newcommand\ShowcaseThemes{%
  \foreach \theme in \WebQuizThemes {
    ...
    \else%
       \ScreenShot[0.68]{Example of the \textbf{\theme} theme}{theme-\theme}%
       \CrossIndex{theme}{\theme}%
    \fi
  }
}

\input{webquiztex.languages} % defines a comma separated list of themes \WebQuizLanguages
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% WebQuizTeX title box for front page
\usepackage{tikz}
...
\def\WebQuizTitle{
  \begin{tikzpicture}[remember picture,overlay]
      \node[yshift=-3cm] at (current page.north west)
        {\begin{tikzpicture}[remember picture, overlay]
          \draw[shadowed](30mm,0) rectangle node[Brown]{\WebQuizTeX} (\paperwidth-30mm,16mm);
          \node[Sienna,font=\normalfont\small\itshape] at (\paperwidth/2,2mm)
          {\small \webquiztex{description}};
          \node[anchor=west,boxes] at (4cm,0cm) {\webquiztex{name}};
          \node[anchor=east,boxes] at (\paperwidth-4cm,0) {Version \webquiztex{version}};
         \end{tikzpicture}
        };
   \end{tikzpicture}
}
...
\def\ps@webquiztex{
  \ps@empty
  \def\@oddfoot{\tiny\WebQuizTeX\space -- version \webquiztex{version}\hfill%
     \textsc{\ifodd\thepage The \WebQuizTeX manual\else \webquizheader\fi}\hfill\thepage}
}
% hijack section and subsectionmark for our headers
\def\webquizheader{\WebQuizTeX}
\def\sectionmark#1{\def\webquizheader{#1}}
\def\subsectionmark#1{\def\webquizheader{#1}}
\pagestyle{webquiztex}
\patchcmd\l@section{1.0em}{0.5em}{}{}
\makeatother

\begin{document}
  \hypersetup{pdftitle={WebQuizTeX manual}}

      \WebQuizTitle

      \begin{quote}
        \WebQuizTeX is a \LaTeX{} package for writing online quizzes. It allows
        ...
        \Javascript. Online quizzes written using \WebQuizTeX can contain any
        material that can be written using \LaTeX, including text,
        mathematics, graphics and diagrams.
        \ScreenShot[0.65]{An example \WebQuizTeX web page}{quiz-page}
        \begin{center}
          \hfil
  ...
  \section{Introduction}
      ...
      \WebQuizTeX makes it possible to write online quizzes using \LaTeX,
      which is the typesetting language used by mathematicians who use
      \LaTeX\ to write their research papers, books and teaching
      materials. In principle, a \WebQuizTeX quiz can contain anything that
      ...
      \WebQuizTeX supports the following three types of questions:
      \begin{itemize}
      ...
      The online quizzes constructed using \WebQuizTeX can, in principle,
      ...
      have to contain ``questions'' as it is possible for a \WebQuizTeX
      ...
      This introduction outlines how to use \WebQuizTeX, however, the
      impatient reader may want to skip ahead directly to the
      \autoref{S:documentclass}, where the \LaTeX\ commands used by
      \WebQuizTeX are described.

      The easiest way to explain how \WebQuizTeX works is by example. The
      ...
      Of course, the real reason for using \WebQuizTeX is to create a
      web page for the quiz, which you do by processing the quiz using
      the \BashCode|webquiztex| command )instead of, say
      ...
      first or subsequent attempts. One of the main points of \WebQuizTeX is
      that (optional) targeted feedback can be given to the student taking
      the quiz based on their answer.

  \subsection{What \WebQuizTeX does and does not do}

      \WebQuizTeX is a tool that makes it possible to write ``interactive''
      online quizzes using \LaTeX{}. To use \WebQuizTeX you only need
      basic working knowledge of \LaTeX{}. In particular, no familiarity
      of the underlying \CSS, \HTML or \Javascript is required.

      \WebQuizTeX can be used to ask students a series of ``quiz''
      questions. In addition, your online quiz web pages can contain
      course material using the \WebQuizTeX \LatexCode|discussion|
      environment; see \autoref{SS:discussion}. You can write \WebQuizTeX
      ...
      (\autoref{SS:classOptions}). One of the key features of \WebQuizTeX is
      ...
      attempted as many times as the student wants. \WebQuizTeX does not
      limit the number of times that questions can be attempted.

      As described in \autoref{SS:Questions}, \WebQuizTeX  supports the following question types:
      \begin{itemize}
        \item Multiple choice questions with a unique correct answer
        \item Multiple choice questions zero or more correct answers
      ...
      \WebQuizTeX supports several different languages and it
      provides a number of different colour schemes (see
      \autoref{SS:commandline} and \hyperref[SS:themes]{Appendix A}).

      \WebQuizTeX quizzes are not timed and they do not have time-limits.

      Quizzes made using \WebQuizTeX are intended to be used as a
      revision resource rather than as an assessment tool. In particular,
      \WebQuizTeX does not provide a mechanism for recording the marks
      ...
      with a database. In addition, if \WebQuizTeX were used as an assessment
      ...
      The questions in a \WebQuizTeX quiz are static. In particular,
      \WebQuizTeX quiz questions do not accept variables.

      The \WebQuizTeX program was designed to be run from the command-line.
      To process the file \BashCode|quiz.tex| using \WebQuizTeX you would
      type
      \begin{bashcode}
        > webquiztex quiz         or         > webquiztex quiz.tex
      \end{bashcode}
      \index{$>$ command-line prompt}\index{command-line prompt!$>$}
      (Throughout this manual, \BashCode|>| is used for the command-line
      prompt.)
      It should be possible to run \WebQuizTeX from inside programs like \TeX
      Shop by setting the compiler equal to \BashCode|webquiztex|, although
      this is untested.

  \subsection{Credits}
      \WebQuizTeX{} was written and developed in the
      ...
      To write quizzes using \WebQuizTeX it is only necessary to know
      \LaTeX, however, the underlying \WebQuizTeX system actually has three
      components:
      \begin{itemize}
        \item A \href{https://www.latex-project.org/}{\LaTeX} document class
        file, \BashCode|webquiztex.cls|, and a \TeXfht
        configuration file, \BashCode|webquiztex.cfg|, that enables the
        quiz files to be processed by \LaTeX{} and \TeXfht, respectively.
        \item A \python program,
        \BashCode|webquiztex|, that translates the
        \LaTeX{} into \XML, using \TeXfht, and then into \HTML.
        \item \CSS, \HTML and \Javascript code controls and style the
        quiz web pages.
      \end{itemize}

     The \LaTeX{} component of \WebQuizTeX{} was written by Andrew Mathas and
     ...
     in use. Prior to releasing \WebQuizTeX on \Ctan, the program was known
     as \texttt{MathQuiz}.
   ...
   \section{The \WebQuizTeX document class --- \hologo{LaTeX}{} commands}
   \label{S:documentclass}

    This chapter describes the commands and environments provided by the
    \WebQuizTeX document class. This assumes that you have already installed
    and configured \WebQuizTeX. If you have not yet initialised \WebQuizTeX
    ...
    found in the \BashCode|examples| subdirectory of the \WebQuizTeX web
    directory.\footnote{After you have initialised \WebQuizTeX, you can find the
    \WebQuizTeX example directory from the command-line using:
    \texttt{webquiztex --settings webquiztex-www}.}
    ...
    Zoomed out, the structure of a typical \WebQuizTeX quiz file is a \LaTeX{} file of the form:
    \begin{latexcode}
      \documentclass{webquiztex}
    ...
    \begin{bashcode}
       > webquiztex quiz    or    > webquiztex -d quiz  # -d = draft mode = faster!
    \end{bashcode}
    The quiz file, \BashCode|quiz.tex|, should be in a directory on your
    web server because \WebQuizTeX creates a number of different files and
    ...
    \begin{itemize}
      \item \textit{Every file that you give to \WebQuizTeX must be a valid
      \LaTeX{} file!}
      ...
      faster} than processing it with \WebQuizTeX. In fact, \WebQuizTeX uses
      \BashCode|htlatex| to process the quiz file at least
      \textit{three times} in order to produce an \XML file and it is only
      then that the \WebQuizTeX program kicks in to rewrite this data as an
      \HTML file. (If you use \textit{draft mode} then
      \BashCode|htlatex| processes the quiz file only once.)

      \item If \LaTeX{} produces errors then \WebQuizTeX will produce more
      errors. Further, \textit{\LaTeX{} error messages are much easier to
      read and understand than those produced by \TeXfht and \WebQuizTeX}.
    \end{itemize}
    This said, \WebQuizTeX does check for more errors in the quiz than
    ...
    \WebQuizTeX for typesetting quizzes as well as the document-class options
    for the package. If you plan to use \ctan{pstricks} or
    \ctan[pgf]{tikz} then you should read \autoref{SS:graphics}, which
    describes how to use graphics in a \WebQuizTeX quiz.
  ...
  The \WebQuizTeX document class defines the following four environments:
  \begin{quote}
    \begin{description}
  ...
  \OnlineManual, in the \WebQuizTeX web directory for a more complete quiz
  file.

  This manual describes the \WebQuizTeX commands, often by example. The
  ...
  correct answer as a string, which it must match exactly.
    ...
    the quiz page generated by \WebQuizTeX looks like:
    ...
    and that \WebQuizTeX accepts $i+7$ as the correct answer.
  ...
  \LatexCode{theme=vibrant}, which changes the \WebQuizTeX colour theme;
  ...
  file, which uses \LatexCode|theme=muted|, through \WebQuizTeX
  ...
  If you have a mixture of \LatexCode|discussion| and \LatexCode|question| environments then
  it is useful to be able to add thinks between them.
  \LaTeX{} provides the \LatexCode|\label| and \LatexCode|\ref| commands
  for cross-referencing, so \WebQuizTeX  builds on this idea to
  \LaTexCode|\dref| and \LaTexCode|\qref| to reference
  \LatexCode|discussion| and \LatexCode|question| environments,
  respectively. Here is an example that shows how they are used:
  \InputLatexCode{discussion-ref}\macroIndex{ref}\macroIndex{dref}\macroIndex{dref}\macroIndex{label}
  which produces the online quiz:
  \ScreenSchot{Crossing-referencing question and discussion environments}{discussion-ref}
  ...
  \subsubsection{Index pages for quizzes}\label{SS:index}
    ...
    folder, on the web server.  The index web page is a \WebQuizTeX file of
    the form:
    ...
    \LatexCode|quizindex| environment. This is because \WebQuizTeX creates
    ...
    \WebQuizTeX provides a straightforward way to place navigation breadcrumbs
    ...
    \end{latexcode}
    in your \LaTeX{} file then \WebQuizTeX will add a corresponding strip
    ...
    \BashCode{webquiztex --edit-settings} to set breadcrumbs
    in the \webquizrc to:
    \begin{latexcode}
    ...
    \begin{description}
      ...
      side-menu above the \WebQuizTeX copyright notice.
      ...
      an quiz page that contained a \WebQuizTeX \LatexCode|quizindex| environment has
      been compiled in the current directory.
      \CrossIndex{breadcrumbs}{quizindex}

      For those interested in how this is done, whenever \WebQuizTeX
    ...
    Here is the list of \WebQuizTeX macros that we be used to set the
  ...
  \begin{description}
    ...
    \Item{Department}\macroIndex{Department}
      ...
      \BashCode|webquiztex --edit-settings|.
  ...
  \webquizrc. After doing this, a typical \WebQuizTeX file might look like this:
  ...
  \subsection{\WebQuizTeX document class options}\label{SS:classOptions}

    The \WebQuizTeX document class supports the following options:
    \begin{latexcode}
  ...
  \begin{description}
    ...
    Set the language used by the \textit{web pages} constructed by \WebQuizTeX.
    The following languages are currently supported by \WebQuizTeX:
    \ListWebQuizLanguages
    The languages files are used to print the various buttons and text
    that is generated on the web pages constructed by \WebQuizTeX. The
    ...
    \begin{latexcode}
      \documentclass[language=German]{webquiztex}
      \documentclass[language=german]{webquiztex}
    \end{latexcode}
    ...
    corresponds to the \WebQuizTeX command-line options
    \BashCode|webquiztex -x| or \BashCode|webquiztex -l|, respectively. The
    ...
    files, which all have names of the form \BashCode|webquiztex-xxx.lang|,
    ...
    \begin{bashcode}
         > kpsewhich webquiztex-english.lang
    \end{bashcode}
    The file \BashCode|webquiztex-english.lang| contains the following:
    \InputBashCode{webquiztex-english.lang}
    ...
    text that is will appear on the \WebQuizTeX web pages. Note that the
    expressions \LatexCode|{}| in the language files should be not be
    removed because they expand to expressions like \LatexCode|(a)|,
    \LatexCode|(b)| etc. when used in the online quizzes.

    To add \WebQuizTeX support for a new language, say language
    \LatexCode|xxx|, copy any \WebQuizTeX language file to a new file
    \BashCode|webquiztex-xxx.lang| and then translate all of the words
    to the right of the equals signs.\footnote{\WebQuizTeX assumes that all language
    names are in lower case so \texttt{xxx}, and not \texttt{XXX},
    should be used.} \WebQuizTeX will be able to find the new language file
    as long as it appears in the \LaTeX{} search path\footnote{If you
    are using \TeXLive then you may need to run \texttt{mktexlsr} from
    an administrators account}. Once the new language file
    \BashCode|webquiztex-xxx.lang| is in the \LaTeX{} search path it can be
    used by \WebQuizTeX using \LatexCode|language=xxx| as a document-class
    option:
    \begin{latexcode}
       \documentclass[language=xxx]{webquiztex}
       \begin{document}
            ...quiz code here...
       \end{document}
    \end{latexcode}
    ...
    language files, as a \textit{new issue} at:
      \href{https://\webquiztex{repository}}{\webquiztex{repository}}.

    \item[theme]\CrossIndex{document class options}{theme}

       \WebQuizTeX\ has a small number of different themes for setting the
       ...
       \WebQuizTeX currently supports the following themes:
        \ListWebQuizThemes
       Example screenshots of all of the \WebQuizTeX themes can be found in
  ...
  It is also possible to include complicated diagrams in \WebQuizTeX
  ...
  By far the easiest way to include images when using \WebQuizTeX is by
  ...
  \LatexCode|\DeclareGraphicsExtensions| to tell \WebQuizTeX the different
  ...
  Note that \WebQuizTeX assumes that all images are \BashCode|SVG| images by
  ...
  Even though \ctan{tikz} and \ctan{pstricks} can be used in \WebQuizTeX
  quizzes, both of these packages sometimes have problems. \WebQuizTeX tries
  ...
  \begin{description}
      ...
      \ctan{pst2pdf}. Applying \ctan{pst2pdf} to a \WebQuizTeX quiz is not
      completely straightforward, so \WebQuizTeX provides the
      ...
      The placement of the image is still not perfect --- although the
      position of the image adjusts with the screen size and it does, in
      fact, display well on a mobile device.  More importantly, if you
      use the \LatexCode|pst2pdf| document-class option then \WebQuizTeX is
      able to display this image.
    ...
    \item[tikz]\CrossIndex{document class options}{tikz}
      ...
      with \TeXfht.  It is important that you use the \WebQuizTeX
      \LatexCode|tikz| document-class option, and not
      \LatexCode|\usepackage{tikz}|, because \WebQuizTeX loads slightly
  ...
  \subsection{Configuring commands and environments for \TeXfht}\label{SS:config}\macroIndex{DisplayAsImage}
    ...
    The underlying engine used by \WebQuizTeX is \TeXfht so, because
    \TeXfht is not able process all \LaTeX{} code, there is \LaTeX{}
    code that \WebQuizTeX is not able to cope with.  This said, \TeXfht is
    able to display \textit{most} \LaTeX{} code and \WebQuizTeX has been
    ...
    is possible to use \ctan[pgf]{tikz} and \ctan{pstricks} in \WebQuizTeX
    quizzes.

    \WebQuizTeX uses \TeXfht{} to convert the quiz content from \LaTeX{} to
    ...
    As a workaround, \WebQuizTeX provides the command
    ...
    not work very well when used in \WebQuizTeX quizzes. As the following
    screenshot shows, if you add the line
    ...
    This web quiz was constructed using the code:
  ...
  \section{System requirements, installation and configuration}
    ...
    \WebQuizTeX takes a \LaTeX\ file and translates it into a functional
    web page. To use \WebQuizTeX the quiz author only needs to know how to
    use \LaTeX\ \textit{and} to have all of the programs used by \WebQuizTeX
    ...
    Behind the scenes, \WebQuizTeX uses \TeXfht, \python, \Javascript and
    several other tools to construct and operate the online quizzes.
    The \WebQuizTeX program has three main components:
      \begin{itemize}
          ...
          %\item Documentation
      \end{itemize}
    Of course, to use the online quizzes created by \WebQuizTeX you
    need a web server. To use \WebQuizTeX all of these files need to
    be in appropriate places.  Fortunately, \Ctan takes care of
    most of this but the web-related files still need to be put onto
    your web server.

    \WebQuizTeX has been tested extensively on Linux and Mac operating
    systems. Several people have used \WebQuizTeX on windows computers,
    but I have not tested the program on a windows computer myself.

  \subsection{System requirements}\index{system requirements}

  In order to work \WebQuizTeX needs the following programs to be
  ...
  \begin{itemize}
     ...
     from \Ctan before you try and use \WebQuizTeX.
      ...
      will need to install the web components of \WebQuizTeX.
  \end{itemize}

  \subsection{Initialising \WebQuizTeX}\label{SS:Initialise}
  \CrossIndex{command-line option}{initialise}
  \index{initialisation}

  \WebQuizTeX is a tool for creating online quizzes and in order for
  ...
  sheets}) need to put onto your web server. \WebQuizTeX has an
  \textit{initialisation} routine for installing the web components
  of the program. In fact, until \WebQuizTeX has been initialised it
  will ask you if you to run the initialisation routine every time
  you use \WebQuizTeX. You can reinitialise \WebQuizTeX at any time using
  the command-line option:
  \begin{bashcode}
    > webquiztex --initialise
  \end{bashcode}
  \WebQuizTeX will actually work without being initialised, however, any
  quiz web pages that are created before initialisation will be
  emblazoned with a message reminding you to initialise \WebQuizTeX.
  ...
  server has been configured. It is essential that the \WebQuizTeX files
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \WebQuizTeX needs to copy several files into a subdirectory of this
  ...
  \begin{itemize}
    \item The location of the \WebQuizTeX web directory, which needs to
  ...
  \BashCode|/usr/local/httpd/| and the \WebQuizTeX web files are in
  the directory \BashCode|/usr/local/httpd/UoS/WebQuizTeX|.  So, I set:
  \begin{quote}
    \begin{tabular}{lll} \WebQuizTeX web
          directory &=& \BashCode|/usr/local/httpd/UoS/WebQuizTeX|\\
          \WebQuizTeX relative URL  &=& \BashCode|/UoS/WebQuizTeX|
      \end{tabular}
  \end{quote}
  Once the initialisation step is complete, \WebQuizTeX is ready to use
  ...
  You can test your \WebQuizTeX installation by compiling the example
  files from the \WebQuizTeX manual. You can find these files in the
  directory \BashCode|web_root/doc/examples|, where the
  \BashCode|web_root| is the directory where you just installed
  the \WebQuizTeX web files. If \Ghostscript and \ctan{dvisvgm} are
  installed and properly configured (see \autoref{SS:graphics}) then you
  should also be able to compile the \OnlineManual using \WebQuizTeX.

  \begin{dangerous}
      \textit{To install the \WebQuizTeX files for general use on
      your system, or to save system wide settings, you need to run
      the initialisation command }\BashCode|webquiztex --initialise|
     ...
     \begin{latexcode}
         > webquiztex --rcfile /path/to/system/rc-file --initialise
     \end{latexcode}\index{rcfile}
  \end{dangerous}

  \begin{heading}[Remark]
    To remove all \WebQuizTeX files from your web server use:
    \begin{bashcode}
        > webquiztex --uninstall
    \end{bashcode}
  \end{heading}
  ...
  \index{pstricks}
  \noindent
  \WebQuizTeX, via \TeXfht, uses \ctan{dvisvgm} to convert certain images to
  ...
  \noindent There are three possibilities:
    ...
    \begin{itemize}
      ...
      case, \WebQuizTeX will not be able to process \ctan{svg} images. You
      need to reinstall \ctan{dvisvgm} with ghostscript support.
    \end{itemize}

  \section{The \WebQuizTeX program} \index{usage}

      The \WebQuizTeX program was designed to be run from the command-line.
      To produce an online quiz from a \WebQuizTeX \LaTeX{} file \BashCode|quiz.tex|
      type:
      \begin{bashcode}
        > webquiztex quiz          or          > webquiztex quiz.tex
      \end{bashcode}
      One feature of \WebQuizTeX is that you can process more than one
      ...
      \begin{bashcode}
        > webquiztex -dqq quiz*.tex
      \end{bashcode}
      because the \BashCode|webquiztex --qq| command-line option suppresses
      ...
      next section discusses the \WebQuizTeX command-line options.

      \subsection{Command-line options}\label{SS:commandline}
      \index{command-line option}
      Typing \BashCode|webquiztex -h|, or \BashCode|webquiztex --help| on
      the command-line gives the following summary of the \WebQuizTeX
      command-line options:
      \InputBashCode{webquiztex.usage}
      ...
      \begin{description}
         ...
         \BashCode|webquiztex -qq texfile.tex| then almost no output will
         be printed by \WebQuizTeX when it is processing your quiz file. Be
         ...
         \begin{description}
            ...
            webquiztex. See \autoref{SS:Initialise} for more details.

            \item[\ddash edit-settings] \CrossIndex{command-line
            option}{edit-settings} Edit the webquiztex settings in the
            \WebQuizTeX rc-file. See \autoref{SS:rcfile} for more details.

            \textit{If you do not have permission to write to the system
            rc-file, which is in the \WebQuizTeX scripts directory, then you
            will be given the option of saving the \WebQuizTeX settings to an
            ...
            on a unix-like system,} use \BashCode|sudo webquiztex --edit-settings|.
            ...
            List system settings for webquiztex stored in \webquizrc, the
            ...
            \InputBashCode{webquiztex.settings}
          ...
          \begin{dangerous}
            ...
            these settings can stop \WebQuizTeX from working.}
          \end{dangerous}
          ...
          \begin{description}
             \item[\ddash make4ht MAKE4HT-OPTIONS]
                ...
                \begin{bashcode}
                          > webquiztex --make4ht "-e file.mk4" myquiz.tex
                \end{bashcode}
                The \BashCode|make4ht| command-line option will be required
                only in rare instances.
                \index{lualatex}\index{xelatex}

            \item[\ddash uninstall] Remove all \WebQuizTeX files from your
                web server directory. This command only removes files that
                \WebQuizTeX may have installed on your web server. It does
                \textit{not} remove \WebQuizTeX from your \LaTeX{} distribution.

           \item[\ddash webquiztex-layout WEBQUIZTEX-LAYOUT]
           \CrossIndex{command-line option}{layout}
              Name of (local) \python code that controls the layout of quiz web page. This
              option is equivalent to setting the \LatexCode|webquiztex-layout|
      ...
      \begin{description}
        \item[\ddash version] Prints the \WebQuizTeX version number and
        exit (currently \webquiztex{version})

        \item[\ddash debugging] Displays extra debugging information
        when compiling and prevents \WebQuizTeX from deleting the many
        intermediary files that are created when building the quiz web
        pages.
      \end{description}

      \subsection{\WebQuizTeX settings and the webquizrc file}
      \label{SS:rcfile}\index{default settings}\index{webquizrc}

      \WebQuizTeX stores the following default sets in \webquizrc*, a
    ...
    \begin{description}[nosep, labelwidth=18ex]
      ...
      \item[version] webquiztex version number for webquizrc settings
                  (\autoref{SS:rcfile})
      \item[webquiztex-url] relative url for webquiztex web directory (\autoref{SS:Initialise})
      \item[webquiztex-www] full path to webquiztex web directory (\autoref{SS:Initialise})
      \item[make4ht] build file for make4ht (\autoref{SS:Initialise})
      \item[mathjax] url for mathjax (\autoref{SS:Initialise})
      \item[webquiztex-layout] name of \python module that formats the quizzes
      (\autoref{SS:layout})
    \end{description}
    ...
    \begin{bashcode}
        webquiztex --edit-settings
    \end{bashcode}
    When changing the settings \WebQuizTeX tries to explain what it is
    ...
    \WebQuizTeX it is recommended that you accept the default values
    for all of the advanced options because it is very unlikely that you
    will need to change these initially.

    The first line of output from \BashCode|webquiztex --settings| gives
    the location of the rc-file being used.  The system rc-file,
    \webquizrc*, is saved in the \BashCode|scripts/webquiztex| subdirectory
    of the \BashCode|TEXMFLOCAL| directory. A typically location for
    this file is
    \begin{bashcode}
        /usr/local/texlive/texmf-local/scripts/webquiztex/webquizrc
    \end{bashcode}
    By default, the \WebQuizTeX settings are saved here so that you do not
    need to reinitialise \WebQuizTeX whenever you update your \TeX{}
    ...
    \end{itemize}
    Each time \WebQuizTeX is run it first reads the system rc-file followed
    by the \textit{users rc-file}, if they exist. When using
    \BashCode|webquiztex --edit-settings| you will be promoted for a
    ...
    use the \BashCode|--rcfile| command-line option:
    \begin{bashcode}
        > webquiztex --rcfile <full path to rc-file> ...
    \end{bashcode}
    If you save the settings to a non-standard location then you will
    need to use the command-line \BashCode|webquiztex --rcfile| to access
    these settings.

  \noindent To describe the \WebQuizTeX defaults settings we consider them by category.
  ...
  \subsubsection*{Institution settings}
    ...
    \subsubsection*{Formatting options}
      ...
      \begin{description}
        ...
        \item[language]\index{language}
          Sets the default language for the \WebQuizTeX web pages. This can
        ...
        \item[theme]\index{theme}
          Sets the default colour theme for the \WebQuizTeX web pages. This can
          be overridden in the quiz file by using the document class
    ...
    \subsubsection*{Advanced options}
      ...
      \begin{description}
        ...
        \item[make4ht]\index{make4ht}
          ...
          directly, \WebQuizTeX uses \ctan{make4ht} to convert the
        ...
        \item[mathjax] \WebQuizTeX web pages use \href{https://www.mathjax.org/}{mathjax} to
    ...
    \subsubsection*{Configuration settings}
       Use \BashCode|webquiztex --initialise| to change these settings.

    \begin{description}

      \item[webquiztex-url] This is the relative URL for \WebQuizTeX web directory
          \CrossIndex{webquizrc}{webquiztex-url}

      \item[webquiztex-www] This is the full path to the \WebQuizTeX web
      directory. The \OnlineManual and other example code can be found
      in the \BashCode|docs| subdirectory. If you use \BashCode|bash|,
      then the command
      \begin{bashcode}
          > cd $(webquiztex --settings webquiztex-www)/docs
      \end{bashcode}%$
      will take you to the \WebQuizTeX online \BashCode|docs/| directory.
      \CrossIndex{webquizrc}{webquiztex-www}

    \end{description}

    \subsection{Changing the layout of the \WebQuizTeX web pages}
    \label{SS:layout}\index{layout}

    \begin{dangerous}
      \textit{This is an advanced \WebQuizTeX feature that most people will
      not need. To change the layout of the quiz web pages created by
      \WebQuizTeX requires working knowledge of \HTML and \python.}
    \end{dangerous}
    ...
    produced by \WebQuizTeX. For example, by adding code to the
    ...
    \WebQuizTeX using the command:
    \begin{bashcode}
       > webquiztex --webquiztex-layout webquiz_myformat quizfile.tex
    \end{bashcode}
    Once the new layout is finalised you can make it the default
    layout by setting \BashCode|webquiztex-layout| equal  to \BashCode|webquiz_my_format|
    in the \webquizrc using \BashCode|webquiztex --edit-settings|.
    \index{edit-settings}\index{webquiztex-layout}

    If you do make modifications to any of these files then, by the
    \WebQuizTeX Licensing agreement, you are required to create a new
  ...
  \subsection{Bugs, issues and feature requests}\index{bug reports}
    ...
    or with \WebQuizTeX. If you can, please also test to see if your code
...
\renewcommand\thesubsection{\Alph{subsection}}
  ...
  \WebQuizTeX{} comes with different themes for changing the colour scheme
  ...
  an \textit{issue} at:
  \begin{quote}
  \href{https://\webquiztex{repository}/issues}{\webquiztex{repository}/issues}.
  \end{quote}
...
\subsection{The online \WebQuizTeX manual}\label{S:online}

  \WebQuizTeX has an \OnlineManual* that is a \LaTeX file written with the
  \LatexCode|webquiztex| document class. The conversion of the manual from
  \LaTeX{} to \HTML is done by \WebQuizTeX. The \BashCode|PDF| version of
  this manual is included here as an easy reference.  The source file
  for the \OnlineManual* is included in the documentation of \WebQuizTeX to
  allow you to create a local version of the \OnlineManual*. Look for the
  file \BashCode|webquiztex-online-manual.tex| in the
  \BashCode|webquiztex-www/docs| directory; see~\autoref{SS:rcfile}.

  The online manual can either be compiled as a PDF file (see below),
  or using \WebQuizTeX to produce an online version of the manual.
  The \OnlineManual* was written for ``internal use'' when \WebQuizTeX was
  first written in 2004. \WebQuizTeX has evolved quite a lot since then.
  ...
  it does not cover some of the more recent features of \WebQuizTeX, such
  ...
  use \BashCode|latex webquiztex-online-manual| to create a
  ...
  \includepdf[pages=-,pagecommand={\pagestyle{webquiztex}}]{webquiztex-online-manual}
...
\vfil
\begin{tabular}{@{}ll}
Authors             & \webquiztex{authors}\\
Description         & \webquiztex{description}\\
Maintainer          & \webquiztex{name}\\
System requirements & \webquiztex{requirements}\\
Licence             & \webquiztex{licence}\\
Release date        & \webquiztex{release date}\\
Repository          & \href{https://\webquiztex{repository}}{\webquiztex{repository}}
\end{tabular}
\eject
