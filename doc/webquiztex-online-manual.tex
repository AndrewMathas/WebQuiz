...
\documentclass[svgnames]{webquiztex}

% load webquiztex-doc code -- the class file loads webquiztex-ini.code.tex is first
...
\usepackage{ifpdf}
\ifpdf
  \PackageError{WebQuizTeX}{This file must be compiled using latex not pdflatex}
\fi
\newcommand{\C}{\mathbb C}
\newcommand*{\vect}[1]{\mathbf{#1}}

\hypersetup{pdftitle={WebQuizTeX online manual}}
...
\begin{discussion}[Introduction]
  \WebQuizTeX is a \LaTeX{} package for creating \textit{interactive}
  web quizzes.  The idea is that you write the quiz using \LaTeX{} and that
  \WebQuizTeX creates the web page from this file. Anything that you can
  write using \LaTeX{} will be converted to \HTML by \WebQuizTeX. This
  includes, for example, mathematics and graphics written using
  \ctan{pstricks}.  \WebQuizTeX supports three different types of quiz
  questions:
 ...
 (See \qref{2})
  ...
  ways. (See \qref{3})
  \end{enumerate}
The use of \WebQuizTeX is described in the next section. Later
sections describe how each of the \WebQuizTeX environments are used.

The \LatexCode|discussion| environment in \WebQuizTeX can also be used to
write Web Pages like this one (The pages you are reading here were
written using \WebQuizTeX.)
\end{discussion}
\begin{discussion}[Basic Usage]
Once you have a \WebQuizTeX file, you can run it through \LaTeX, in
the usual way, to produce a readable version of your quiz. When you
are happy with the quiz, use \WebQuizTeX to create the HTML
...
\begin{latexcode}
      ...
      > webquiztex quiz1           % converts the quiz to html
\end{latexcode}
  Converting the quiz to html can take quite a long time, particularly
  if a large number of images need to be created.

\end{discussion}
\begin{discussion}[WebQuizTeX files]

  The basic structure of a \WebQuizTeX file is as follows:
\begin{latexcode}
...
\end{latexcode}
  ...
  \WebQuizTeX supports three types of questions:
  \begin{itemize}
...
\end{latexcode}
  ...
  \par
  For example, the code below, when run through \WebQuizTeX, produces \qref{1}
...
\end{latexcode}
  ...
  \par
  For example, \qref{2} below was typed into \WebQuizTeX
...
\end{latexcode}
  ...
  \par
  For example, \qref{3} below was typed into \WebQuizTeX
  using the following commands:
\begin{latexcode}
    \begin{question}
...
\begin{discussion}[Question 4]
  The final type of question that \WebQuizTeX supports is a question that
  ...
  \begin{latexcode}
         ...
         \end{question}
  \end{latexcode}
  See the \WebQuizTeX manual for details of the different
  comparison types.  For example, \qref{4} below was typed into
  \WebQuizTeX using the following commands:
\begin{latexcode}
    \begin{question}
...
\begin{discussion}[Index Files]
  \WebQuizTeX also provides a mechanism for creating a web page
  containing an index of all quizzes for a given Unit of Study.
  This is done with a \WebQuizTeX file that contains a \LatexCode|quizindex|
...
\LatexCode|\quiz| then
  \WebQuizTeX sets the url(s) to \BashCode|quiz1.html|, \BashCode|quiz2.html|
  and so on.
\end{discussion}
...
\begin{discussion}[Credits]
    ...
    \LaTeX, however, the \WebQuizTeX system has three components:
    \begin{itemize}
      \item A \LaTeX{} document class file, \LatexCode{webquiztex.cls}, and
      ...
      \item A python program, \BashCode{webquiztex}, that translates the \XML
...
%% End of file `webquiztex-manual.tex'.
