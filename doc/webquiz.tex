...
\begin{document}
  ...
  \LaTeX{} \textit{label}, defined using the \LatexCode|\latex| command,
  to insert a button or a link whereas \LatexCode|\Qref| uses the actual
  ...
  quite similar they serve quite different functions when the
  \LatexCode|randomorder| document class option is used; see
  \autoref{S:documentclass}.  In this case we do not know ahead of time
  ...
  question \textit{but} this will almost certainly not be question one.
  On the other hand, we can create a ``Start quiz'' button, for example,
  that will open question~1 on any quiz using \LatexCode|\Qref[Start quiz]{1}|.

  \begin{dangerous}
    When using the \LatexCode|randomorder| document class option,
    ``optional text'' should always be given when using the
    \LatexCode|\qref| command because otherwise the displayed question
    number will often be wrong.
  \end{dangerous}

  Here is an example showing how \LatexCode|\dref| and \LatexCode|qref|
