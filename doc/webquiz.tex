...
\NewDocumentCommand\OnlineManual{s}{%
  \href{http://www.maths.usyd.edu.au/u/mathas/WebQuiz/webquiz-online-manual.html}{OnlineManual}%
...
\begin{document}
  ...
  \subsection{Credits}
   ...
   \label{S:documentclass}
    ...
    \begin{bashcode}
       > webquiz quiz    or    > webquiz -d quiz  # -d = draft mode = quicker!
    \end{bashcode}
    ...
    directories when it converts the file into an on-line quiz and all
  ...
  \subsubsection{Index pages for quizzes}\label{SS:index}
    ...
    \begin{description}
      ...
      automatically added to quiz webpages that have a
  ...
  \begin{description}
    ...
    \CrossIndex{document class options}{randomorder}
      ...
      file for the quiz. That is, the first question in the on-line quiz
      ...
      that the quiz is run. With this option, the on-line quiz questions
      ...
      the fourth on-line quiz question. The next time the page is
    ...
    characters then we recommend using Xe\LaTeX{} or Lua\LaTeX, which
    ...
    text that is will appear on the \WebQuiz web pages. Note that the
    expressions \LatexCode|{}| in the language files should be not be
    removed because they expand to expressions like \LatexCode|(a)|,
    \LatexCode|(b)| etc. when used in the on-line quizzes.

    To add \WebQuiz support for a new language, say language
    \LatexCode|xxx|, copy any \WebQuiz language file to a new file
    \BashCode|webquiz-xxx.lang| and then translate all of the words
    to the right of the equals signs.\footnote{\WebQuiz assumes that all language
    names are in lower case so \texttt{xxx}, and not \texttt{XXX},
    should be used.} \WebQuiz will be able to find the new language file
    ...
    \item[theme]\CrossIndex{document class options}{theme}
       ...
       \WebQuiz currently supports the following themes:
        \ListWebQuizThemes
       Example screenshots of all of the \WebQuiz themes can be found in
  ...
  \ctan{pstricks}. \textit{As there have been several recent updates to
  these packages it is advisable to install the latest version of both
  of these packages, as well as the packages \ctan{make4ht}, \ctan{pgf}
  and \TeXfht.} In fact, it is recommended that you update all installed
  \TeX{} packages.
  ...
  \begin{description}
      ...
      this adjusts with the screen size and it does, in fact, display
    ...
    \item[tikz]\CrossIndex{document class options}{tikz}
      ...
      packages updated.} Thanks are due to Michal Hoftich for teh
  ...
  \TeXfht}\label{SS:config}\macroIndex{DisplayAsImage}
    ...
    not work very well when used in \WebQuiz quizzes. As the following
...
\subsection{The on-line \WebQuiz manual}\label{S:online}
  ...
  The on-line manual can either be compiled as a PDF file (see below),
  or using \WebQuiz to produce an on-line version of the manual.
