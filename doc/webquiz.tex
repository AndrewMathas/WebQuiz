...
\subsection{What WebQuiz does and does not do}
    ...
    \WebQuiz is a tool that makes it possible to write ``interaactive''
    on-line quizzes using \LaTeX{}. Only rudimentary knowledge of
    \LaTeX{} is needed and, in particular, no knowledge of the
    underlying HTML CSS, or javascript is required. \WebQuiz can be used
    to ask students a series of ``quiz'' questions.

    By default in the on-line version of the quiz, the quiz question are
    displayed one at a time it is also possible to display all questions
    at once. Each question in a quiz, and each quiz itself, can be
    ...
    the \LaTeX{} file for the quiz or in random order. For multiple
    choice questions the order in which the choices appear is
    fixed, even if the questions appear in random order.
    ...
    access the answers if you know what you are doing.
    ...
    exactly the same order each time the quiz is taken.
...
\subsection{Credits}
    ...
    \begin{itemize}
      ...
      file, \BashCode|webquiz.cls|, and a \ctan[tex4ht]{\TeX 4ht}
      configuration file, \BashCode|webquiz.cfg|, that enable the
      ...
      code that works behind the screens to control and style the quiz
      web pages.
    \end{itemize}
 ...
 \label{S:documentclass}
  ...
  the content of the quiz, say \BashCode|quiz.tex|, then you can convert
  ...
  The next sections describe the document class options and the
  different environments and commands provided by \WebQuiz. The last
  section in this chapter, \autoref{SS:graphics}, lists the class
  options for the \WebQuiz document class. You should read this section
  if you plan to use \ctan{pstricks} or \ctan[pgf]{tikz}.

  Finally, if you want to use \WebQuiz for your students then you will
  need to \textit{initialise} \WebQuiz using the command:
  \BashCode|webquiz --initialise|; see \autoref{SS:Initialise} for more
  details.
...
document class options. Defaults for all of these options can be set in
the \webquizrc file.

  \begin{description}
    \item[fixedorder] Display the quiz questions in the order that they
    appear in the \LaTeX{} file. This is the default behaviour.
    \item[hidesidemenu]
    ...
    \item[language]
      ...
      \texttt{english}
      \item[pst2pdf] Preprocess the quiz using \ctan{pst2pdf}. This
      sometimes fixes issues when using \ctan{pstricks}
    \item[randomorder]
      Display the quiz questions in the order that they
      appear in the \LaTeX{} file. This is the counterpart to
      \texttt{fixedorder}
    \item[sidemenu]
      Display the side menu when the quiz starts, providing that the viewport is large enough. This is the counterpart
      to \texttt{hidesidemenu}. This is the default behaviour
    \item[theme]
      Set the quiz colour scheme.
    \item[tikz]
  \end{description}

  All other class options that are given to the \WebQuiz document class
  are passed to the \LatexCode{article} class, which is the base class
...
\begin{description}
  ...
  The currently supported languages are:
    ...
    save the file as \BashCode|webquiz-XXX.lang|, and then put this file
  ...
  \item[theme]\CrossIndex{document class options}{theme}
    ...
    By default, the questions in the quiz are displayed in the order
    that they appear in the \LaTeX{} file for the quiz. With the
    \textt{randomorder} option the questions in the quiz will be
    displayed in a random order each time that the quiz is run. For
    example, the code:
    \InputLatexCode{examples/random}
    will randomly permute the questions each time the quiz web page is
    loaded. This produces the (random) quiz:
    \ScreenShot{Randomly ordered questions}{examples/random}

    \item[sidemenu, hidesidemenu]\CrossIndex{document class options}{hide side menu}
...
question. See the on-line manual (\autoref{S:online}), in the \WebQuiz web
directory for a complete quiz file.
  ...
  The macros \LatexCode|\whenRight| and \LatexCode|\whenWrong| are used
  to give the student feedback, when they are right, or further hints
  etc for approaching the question, when they are wrong.

  The web page created by the code above, when a correct answer is
  given, looks like this:

  \ScreenShot{A question with an integral answer}{examples/answer-integer}
  ...
  \begin{description}
      \item[integer] The \textit{correct answer} and the inputted answer
      as compared as \textit{integers}. This is the default.
      \CrossIndex*{answer}{integer}
      \item[number] The \textit{correct answer} and the inputted answer
      ...
      \item[string] The \textit{correct answer} and the inputted answer
      as compared as \textit{strings}. Note that \textit{string} is case
      sensitive.
      \CrossIndex*{answer}{string}
      \item[lowercase] The \textit{correct answer} and the inputted answer
      are converted to lower case and then compared as strings.
      \CrossIndex*{answer}{eval}
      \item[eval] The inputted answer is marked correct if the \textit{difference}
  ...
  \ScreenShot{A question with a string answer}{examples/answer-string}

  \textcolor{blue}{Lowercase string answer}
  ...
  \textcolor{blue}{Evaluated answer}
  \CrossIndex*{answer}{eval}
  \InputLatexCode{examples/answer-eval}
  \ScreenShot{A question with an evaluated answer}{examples/answer-eval}
...
discussion.
  ...
  either beings with a backslash, $\backslash$, or with \BashCode|http|.
...
\begin{description}
  \Item{BreadCrumb}\macroIndex{BreadCrumb}
    ...
    the part of the quiz title, as set by \LatexCode|\title|, before the
    first colon. For example, the title

     \begin{latexcode}
       \title{Quiz 1: Some interesting questions about frogs}
     \end{latexcode}
  ...
  \Item{Department}\macroIndex{Department}
    ...
    Default value: \DefaultValue{Mathematics}
  ...
  \Item{DepartmentURL}\macroIndex{DepartmentURL}
    ...
    Default value:  \DefaultValue{QuizURL/Quizzes}, where \DefaultValue|QuizURL\ is set
...
\section{System requirements, installation and configuration}
  ...
  \CrossIndex{system requirements}{python}
    ...
    \begin{bashcode}
      > webquiz --initialise
    \end{bashcode}
    Initialising \WebQuiz also allows you to set various system
    ...
    \textit{using an administrator account or using \textsf{sudo} on a
    ...
    \begin{itemize} \item The location of the \WebQuiz web directory,
        which needs to be a directory on your local file system that is
        visible to your web server \item The relative URL for this
        directory, which tells your web browser where to find these
        files \end{itemize} For example, on my system the base directory
        for our web server is \textsf{/usr/local/httpd/} and the
        \WebQuiz web files are in \textsf{/usr/local/httpd/UoS/WebQuiz}.
        So, I set: \begin{quote} \begin{tabular}{lll} \WebQuiz web
            directory &=& \textsf{/usr/local/httpd/UoS/WebQuiz}\\
            \WebQuiz relative URL  &=& \textsf{/UoS/WebQuiz}
        \end{tabular} \end{quote} In addition, the initialisation
        command lets you set global defaults for the following:
        \begin{itemize}
            \item breadcrumbs = the breadcrumbs at the top of the quiz
            web page
            \item colour theme = default colour theme for web pages
            \item department = a (short) name for your department
            \item department-url = the URL for your department's web server
            \item engine = default \TeX{} engine for compiling quizzes
                  (latex, xelatex or lualatex)
            \item institution = a (short) name for your institution
            \item institution = the URL for your institution
            \item language = default language for quizzes
        \end{itemize}
        There are three additional advanced options. For more details see
        \autoref{SS:rcfile}, which describes the \webquizrc file.  The
        default values of all of these settings can be overridden in the
        \LaTeX{} file for the  quiz and the default values can be
        changed at any time using \BashCode|webquiz --edit-settings|.

     When initialising \WebQuiz if you are unsure what
     a particular setting does then you should just hit return to accept
     the default value for setting --- the default value is printed
...
\subsection{The on-line WebQuiz manual}\label{S:online}
  ...
  \BashCode|webquiz-online-manual.tex| in the \textsf{webquiz-www/docs}
