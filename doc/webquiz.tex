...
\usepackage{manfnt}
\usepackage{booktabs}
\usepackage{bbding}\let\ding\relax
...
\WebQuiz manual.  It does not index the \OnlineManual.}}
  ...
  \section{Introduction}
      ...
      like this:
      \ScreenShot[0.5]{Sample \BashCode|PDF| output}{examples/simple-pdf}
  ...
  correct answer as a string, which it must match exactly.
    ...
    \ScreenShot{A question with answer$*$}{examples/answer-star}
  ...
  \ScreenShot[0.7]{Web page: discussion
  environment}{examples/discussion} As with the questions, only one
  ...
  \subsubsection{Index pages for qizzes}\label{SS:index}
    ...
    \InputLatexCode{examples/index-en}
    which generates a web page that looks like:
    \ScreenShot{Example index page}{examples/index-en}
    ...
    \InputLatexCode{examples/index-cz}
    produces:
    \ScreenShot{A Czech index}{examples/index-cz}
    ...
    \begin{description}
      ...
      \ScreenShot{Drop-down menu giving index of
      quizzes}{examples/dropdown} \macroIndex{QuizzesURL}
  ...
  it is recommended that you update all of your packages.
  ...
  quizzes, unfortunately, both of these packages sometimes have problems
  when used with \TeXfht and hence with \WebQuiz. WebQuiz tries to solve
  some these problems for you provided that you use the appropriate
  document class options.

  For example, the following \ctan{pstricks} code works out of the box:
    \InputLatexCode{examples/pstricks-ex}
  to produce the web page:
    \ScreenShot{Pstricks example}{examples/pstricks-ex}
  ...
  \begin{description}
      ...
      When they do fail they can often be salvaged by using
      \ctan{pst2pdf}. Applying \ctan{pst2pdf} to a \WebQuiz quiz is not
      completely straightforward so \WebQuiz provides the document-class
      option \ctan{pst2pdf} to automatically use \ctan{pst2pdf} as
      part of the quiz web page build process. Using \ctan{pst2pdf} will
      often, but not always fix problems when \ctan{pstricks} images do
      not display correctly on a \WebQuiz quiz web page.
      ...
      warnings and it needs to be properly installed.
      ...
      \begin{quote}
        ...
        using \ctan{pst2pdf}.
      \end{quote}
      If using \ctan{pst2pdf} does not produce an image then the
      problem might be that you have not installed all of the programs
      that \ctan{pst2pdf} relies upon, so look in your log files
      for error messages and check that the programs listed above are
      correctly installed. See also \autoref{SS:pstricks}.
  ...
  Most people use either \ctan{pstricks} or \ctan{tik}. A quiz that
    ...
    \CrossIndex{system requirements}{python}
      ...
      \begin{itemize}
         ...
         before you try and use \WebQuiz.
          ...
          \item \python[Python 3]. As of writing python 3.7 is available.
          \item \Javascript
