...
discussion.
  ...
  \subsubsection{Index pages for qizzes}
  ...
  breadcrumbs are disabled. Adding \LatexCode|\BreawdCrumbs| command
  like:
  \begin{latexcode}
  ...
  \noindent
  The breadcrumbs will typically be links. In the example above, clicking on
  \textsf{Mathematics} takes you to the web page for the
  department of mathematics, clicking on \textsf{MATH1001} takes you to the
  unit of study web page and clicking on \textsf{Quizzes} takes you
  ...
  \macroIndex{BreadCrumbs} as above, or by setting the
  \textsf{breadcrumbs} ``globally'' in the \webquizrc file using the
  command-line option
  \begin{bashcode}
...
\webquizrc file; see \autoref{SS:rcfile}.
  ...
  \begin{description}
    ...
    \item[pst2pdf]
    Pre-process the quiz using \ctan{pst2pdf}. This sometimes fixes
    issues when using \ctan{pstricks}; see \autoref{SS:graphics} for
    more details
    ...
    \item[tikz]
    This loads \ctan{tikz} and fixes some known issues when using
    \ctan{tikz} with \TeXfht
...
\begin{description}
  ...
  \item[theme]\CrossIndex{document class options}{theme}
    ...
    \LatexCode|randomorder| option the questions in the quiz will be
