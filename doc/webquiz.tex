...
\begin{document}
  ...
  none of the incorrect choices, are selected. When the student answer
  is incorrect then they they are given feedback that is randomly
  selected from the wrong choices (that is, the feedback is
  randomly selected from the set of \LatexCode|\correct| choices that
  were not selected and the \LatexCode|\incorrect| choices that were
  selected).
  ...
  question choices are labelled in the on-line quiz. For example, to
  ...
  \section{The WebQuiz program} \index{usage}
    ...
    the location of the rc-fuke being used.  The system rc-file,
