...
\csdef{TeX4ht}{\ctan[tex4ht]{\TeX 4ht}}
\newcommand\TeXLive{\ctan[/]{\TeX Live}}
...
The \textbf{mathquiz} document class: currently supports the following
five document class options:
    ...
    \begin{latexcode}
       ...
       \documentclass[pst2pd]{webquiz} % automatically apply \ctan{pst2pdf}
    \end{latexcode}
...
\begin{description}
  ...
  The currently supported languages are:
    ...
    they may well need fine-tuning. You can use \textsf{kpsewhich} to
    ...
    \begin{quote}
         \textsf{kpsewhich} webquiz-english.lang
    \end{quote}
    This file contains the following:
    \lstinputlisting[extendedchars=false]{webquiz-english.lang} In these
    files, the material to the left of the equals signs are
    ...
    \Verb|({})| in the language files should be not be removed because
    ...
    used by \WebQuiz in the obvious way:
    \begin{latexcode}
  ...
  \item[theme]\CrossIndex{document class options}{theme}
     ...
     \WebQuiz\ has a \Verb|light| and a \Verb|dark| theme for styling
     the quiz web pages, which can be set as options to the document
     class or in the \webquizrc file. All of the examples so far have
     used the \Verb|light| theme, which is the default. As an example,
     if we use

    \lstinputlisting[style=latexcode]{examples/choice-multiple-dark}

    then the quiz page looks like:

    \ScreenShot{Multiple choice with light theme}{examples/choice-multiple-dark}
...
Diagrams and graphics in \LaTeX\ documents are often included using
either \ctan{graphicx}, \ctan{tikz} or \ctan{pstricks}. These packages
can also be used in \WebQuiz quizzes although, unfortunately, both of
these packages have issues when used with \TeX 4ht. The good news is
that \WebQuiz attempts to solve these problems for you if you use the
appropriate document class options.
...
\begin{description}
  ...
  \item[tikz]\CrossIndex{document class options}{tikz}
    ...
    \Verb|\usepackage{tikz}| in your \LaTeX\ file). As a bonus, thjis
    \Verb|tikz| document option fixes several issues with PGF that
...
\subsection{Configuring commands and environments for \TeX4ht}
  ...
  can sometimes be tricky to do. An easy work-around that usually
  produces good results is to
...
\section{System requirements, installation and configuration}
  ...
  \CrossIndex{system requirements}{python}
    ...
    \begin{itemize}
       ...
       by \TeXLive. In particular, you need to have \TeX4ht{} installed.
       It is recommend that you have the equivalent of \TeXLive{} 2018,
       or later, installed, because of recent changes to \TeX4ht{}.
        \item \href{https://www.python.org/}{Python 3}. As of writing
        python 3.7 is available.
        \item Javascript
        \item If your quizzes incorporate graphics using \ctan{pstricks}
    ...
    \subsection{Pstricks configuration and the on-line manual}
...
The short version is you should look at the output of:
...
There are three options:
    ...
    ghostscript was linked at build time if the -h output does not contain -libgs and the -l output contains ps
    postscript support is enabled but ghostscript is not linked if the -h output contains -libgs and the -l output does not contain ps
    dvisgm weas not built with postscript support if the -h output does not contain -libgs and the -l output does not contain ps

My guess is that you are in either the second or third case. In the second case you just need to locate the ghoshtscipt library libgs.so or libgs.dylib and set the LIBGS environment variable.
    ...
    When you run the \WebQuiz initialisation command,
     \webquizopt{initialise},  you will be prompted for the following:
     \begin{itemize}
...
option.
  ...
  WebQuiz settings stored in /Users/andrew/.webquizrc
    ...
    mathjax         = https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.0/
       ...
       \item[\ddash webquiz\_format WEBQUIZ\_FORMAT] \CrossIndex{command-line options}{layout}
    ...
    \begin{description}
        ...
        \item[language]\index{language}
          ...
          \textsf{language} option: \Verb|language=XXX|
        ...
        \item[theme]\index{theme}
          ...
          be overridden in the quiz file by using the document class
          \textsf{theme} option: \Verb|theme=XXX|.
          See \autoref{SS:classOptions}.
  ...
  \subsubsection{Changing the layout of the \WebQuiz web pages}\index{layout}
