...
\begin{description}
  ...
  \item[theme]\CrossIndex{document class options}{theme}
     ...
     so far have used the \Verb|light| theme, which is the default. As
     an example, if we use

    \lstinputlisting[style=latexcode]{examples/theme-dark}
...
\subsection{List of themes}\index{SS:themes}
\WebQuiz{} comes with quite a few different themes for the on-line
quizzes, which can be set using the \textsf{theme} documentclass option
or in the \webquizrc file; see \autoref{SS:classOptions} for more
details. Themes are easy to construct in principle although finding
colour that work well together can be tricky in practice.
