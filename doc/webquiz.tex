...
\subsection{Credits}
 ...
 \section{The WebQuiz document class --- \LaTeX{} commands}\label{S:documentclass}
  ...
  using the command: \lstinline[style=bashcode]|webquiz --settings webquiz-www|.}
...
discussion.
  ...
  \begin{bashcode}
        \BashCode|webquiz --edit-settings|\qquad (see \autoref{SS:commandline}).
  \end{bashcode}
  ...
  \begin{bashcode}
     department | unitcode | quiz\_index | breadcrumb
  \end{bashcode}
...
to \LatexCode|graphicx| is necessary if you want to be able to rescale
...
\subsection{Configuring commands and environments for \TeXfht}
  ...
  that defines a macro \LatexCode{\ce}
  for writing chemical symbols but, unfortunately, the \LatexCode{\ce}
  macro has not (yet) been configured to
  work with \TeXfht, which means that this command will not in \WebQuiz quizzes.
  Putting the command \LatexCode{\DisplayAsImage{ce}} into your
  preamble fixes this and allows to you to produce quizzes using
  \LatexCode{ce}. For example, you can produce the quiz

  \ScreenShot{Using \LatexCode{\DisplayAsImage}}{examples/display-as-image}
...
\section{System requirements, installation and configuration}
  ...
  \CrossIndex{system requirements}{python}
    ...
    \subsection{Pstricks configuration and the on-line
    manual}\label{SS:pstricks}
...
\end{verbatim}
to my \BashCode|.bashrc| file. You may need to do something similar.
...
\section{The WebQuiz program} \index{usage}
    ...
    \noindent
    because the \BashCode|webquiz --qq| command-line option suppresses almost all of
    the output produced by \LaTeX\ and \TeXfht. The next section
...
option.
    ...
    \begin{description}
       ...
       \item[\ddash make4ht MAKE4HT\_OPTIONS] \CrossIndex{command-line options}{make4ht}
          ...
          option is equivalent to setting the \BashCode|make4ht|
          ...
          \BashCode|--shell-escape|, \BashCode|-lua|, \BashCode|--xelatex|, so this command-line
          option will be required only in very rare instances.
    ...
    \begin{description}
        ...
        \item[webquiz\_www] This is the full path to the webquiz web
            ...
            then the command
            \begin{bashcode}
                    > cd \$(webquiz --settings webquiz\_www)/docs
            \end{bashcode}
    ...
    The values of \LatexCode|breadcrumbs|, \LatexCode|department|, \LatexCode|department_url|,
    \LatexCode|institution| and \LatexCode|institution_url| can all be overwritten
    using the corresponding  \LaTeX{} commands in the quiz file; see
    \autoref{SS:breadcrumbs} for more details.


  \subsubsection{Changing the layout of the \WebQuiz web pages (advanced)}\index{layout}
  ...
  function \PythonCode|write\_web\_page| that returns the HTML for the page
  ...
  format using
  \BashCode|webquiz --edit-settings|.\index{edit-settings}\index{webquiz\_format}
