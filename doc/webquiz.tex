...
discussion.
  ...
  quiz-by-quiz, using the \Verb|\BreadCrumbs| macro
...
adding
\begin{center}
\LaTeXCode|\usepackage[dvipdfmx]{graphicx}\DeclareGraphicsExtensions{.png}|
\end{center}
to your document preamble. You need to use
\LaTeXCode|\DeclareGraphicsExtensions| to tell \WebQuiz what types of
images you are using (you can you can use a comma separated list of
extensions like png, jpg, gif, etc). The option \Verb|dvipdfmx|
to \Verb|graphicx| is necessary if you want to be able to rescale
images. For example, the code:

    \lstinputlisting[style=latexcode]{examples/using-images}
...
\section{The WebQuiz program} \index{usage}
    ...
    \subsection{Command-line options}\label{SS:commandline}\index{command-line options}

    \lstinputlisting[language=bash]{webquiz.usage}
...
option.
    ...
    \begin{description}
       ...
       \item[\ddash settings {[SETTING]}] \CrossIndex{command-line options}{settings}
          ...
          \lstinputlisting[language=bash]{webquiz.settings}
