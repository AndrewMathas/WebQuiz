...
%\lstset{inputencoding=utf8/cp1250}
...
\newcommand\ShowcaseThemes{%
  \foreach \theme in \WebQuizThemes {
    ...
    \else%
       \ScreenShot[0.65]{Example of the \textbf{\theme} theme}{theme-\theme}%
...
\begin{document}
  \hypersetup{pdftitle={WebQuiz manual}}
      ...
      \begin{quote}
        \WebQuiz is a \LaTeX{} package for writing on-line quizzes. It allows
  ...
  \subsection{What WebQuiz does and does not do}
      ...
      on-line quizzes using \LaTeX{}. To use \WebQuiz you only need
      ...
      questions. In addition, your on-line quiz web pages can contain
      ...
      \LatexCode|discussion|, and (psuedo) quizzes that contains only
  ...
  \subsection{Credits}
   ...
   \label{S:documentclass}
    ...
    \begin{latexcode}
      ...
      \end{document}
    \end{latexcode}
    You should write your quizzes using the editor that you
  ...
  \begin{itemize}
    ...
    \textit{Use \LatexCode|columns=n| with care when $n>1$ as mutiple
  ...
  \LatexCode|\feedback| \CrossIndex{choice environment}*{response} to give
  ...
  \subsubsection{Index pages for qizzes}\label{SS:index}
    ...
    \LatexCode|breadcrumbs|  in the \webquizrc using the
    command-line option
    \begin{bashcode}
    ...
    \begin{description}
      ...
      \CrossIndex*{BreadCrumbs}{quizindex} This drop-down menu will only
    ...
    Here is the list of \WebQuiz commands that can be used to specify
    the contents of some of the ``magic'' breadcrumbs. Default values
    for many of these ``crumbs'' can be given in the \webquizrc.
  ...
  \subsection{WebQuiz document class options}\label{SS:classOptions}
    ...
    for the quiz override the default settings in the \webquizrc.
  ...
  \begin{description}
    ...
    \CrossIndex{document class options}{showsidemenu}
      ...
      by clicking on the {\textcolor{red}{\small\XSolidBold}} and
      {\textcolor{red}{\large\ding{118}}} symbols. The side-menu automatically
      disappears for devices, such as mobile phones, with narrow
      screens.
    ...
    default \TeX{} engine can be set in the \webquizrc; see
    ...
    \item[theme]\CrossIndex{document class options}{theme}
       ...
       the document class or in the \webquizrc. Most of the examples
  ...
  Note that \WebQuiz assumes that all images are \BashCode|SVG| by
  ...
  \subsection{Initialising \texorpdfstring{\WebQuiz}{WebQuiz}}\label{SS:Initialise}
  ...
  \section{The WebQuiz program} \index{usage}
      ...
      would probably use
      \begin{bashcode}
        > webquiz -dqq quiz[1-9].tex
      \end{bashcode}
      because the \BashCode|webquiz --qq| command-line option suppresses
      almost all of the output produced by \LaTeX\ and \TeXfht and
      \BashCode|-d| is faster and it is probably sufficient if the
      quizzes were compiled recently. The next section discusses the
      ...
      \begin{description}
         ...
         \item[-i, \ddash initialise] \CrossIndex{command-line option}{initialisation}
            Initialise files and settings for webquiz. See
            \autoref{SS:Initialise} for more details.

         \item[\ddash edit-settings] \CrossIndex{command-line option}{edit-settings}
            Edit the webquiz settings in the \WebQuiz rc-file. See
            \autoref{SS:rcfile} for more details.
       ...
       %    necessary if you want to override the default rc-file.
          ...
          \textit{Change these settings with are: an incorrect value for
      ...
      \subsection{\texorpdfstring{\WebQuiz}{WebQuiz} settings and the webquizrc file}
    ...
    So that settings are not lost when \WebQuiz, or \TeX Live is
    upgraded, the system rc-file, \webquizrc*, is saved in the
    \BashCode|scripts/webquiz| subdirectory of the \BashCode|TEXMFLOCAL|
    directory. A typically location for this file is
    \begin{bashcode}
        /usr/local/texlive/texmf-local/scripts/webquiz/webquizrc
    \end{bashcode}
    If you are using \TeXLive (and possibly Mik\TeX?) you can find this
    ...
    \begin{itemize}
      \item \BashCode|~/.dotfiles/config/webquizrc| if the directory
             \BashCode|.dotfiles/config| exists
      \item \BashCode|~/.config/webquizrc| if the directory \BashCode|.config| exists
      \item  and otherwise to\BashCode|~/.webquizrc|.
    \end{itemize}
    Each time \WebQuiz is run it first reads the system rc-file, if it
    exists, followed by the \textit{users rc-file}, if it exists. If you
    do not have permission to write to the specified rc-file then you
    will be asked for a different install location.

    To describe these settings in more detail we break them up by
    category.
    ...
    \subsubsection*{Formatting options}
      ...
      \begin{description}
        ...
        \item[engine] Sets the default \TeX{} engine to be used when
          ...
          \webquizrc* file can be overridden by the webquizrc
    ...
    \subsection{Changing the layout of the \texorpdfstring{\WebQuiz}{WebQuiz} web pages}
...
\renewcommand\thesubsection{\Alph{subsection}}
  ...
  quizzes, which can be set using the \LatexCode|theme| document-class
  option or in the \webquizrc; see \autoref{SS:classOptions} for
  ...
  \ShowcaseThemes
  Any new I am happy to include any new themes sent to me and to consider
...
\subsection{The on-line WebQuiz manual}\label{S:online}
  ...
  done by \WebQuiz. The \BashCode|PDF| version of this manual is included here
  as an easy reference.  The source file for the \OnlineManual is included
  ...
  There is some overlap between the \OnlineManual and
  ...
  features of \WebQuiz, such as the document class options, or how to use the program.  If there are any
  ...
  \BashCode|PDF| using \BashCode|dvipdf|. The on-line manual needs to be
