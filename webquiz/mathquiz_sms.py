...
def initialise_SMS_Menus(web_page):
    if web_page.course['code'] in ["MATH1002", "MATH1005"]:
        content = "MATH100{0}<br>MATH190{0}<br>Quizzes".format(
            web_page.course['code'][-1])
    else:
...
def SMS_menu(doc):
    ...
    if len(doc.quiz.course['name']) > 0:
        ...
        if len(doc.quiz.quiz_list) == 0:  # not on a quiz index page
            ...
            menu = navigation_menu.format(
                menu=type,
                heading='' if heading == '' else '<li>{}</li>'.format(heading))
        else:
            menu = ''
    else:
    ...
    if len(doc.quiz.discussion_list) > 0:  # links for discussion items
        discussion_list = '\n       <ul>\n   {}\n       </ul>'.format(
            '\n   '.join(
                discuss.format(b=q + 1, title=d.short_heading)
                for (q, d) in enumerate(doc.quiz.discussion_list)))
    else:
        discussion_list = ''
    ...
    buttons = '\n' + '\n'.join(
        button.format(
            b=q,
            cls=' button-selected'
            if len(doc.quiz.discussion_list) == 0 and q == 1 else '')
    ...
    return menu_name, sms_side_menu.format(
...
def write_web_page(doc):
    ...
    page = dict(
        ...
        nopreview='',
        menu_icon=
        '<div class="menu_icon" onclick="toggle_side_menu_display();">&#9776;</div>'
    )
    for (key, value) in [
        ('CODE', 'QUIZ'), ('menuname', sms_menu_name), ('pagetitle',
                                                        doc.title),
        ('title', ''), ('extrajsmenu', 'webquiztitles'),
        ('tablevel',
         'internal' if doc.quiz_file in ['webquiz-manual', 'credits'] else '')
    ]:
    ...
    return sms_write_page(page, {}, doc.course['quizzes_url'][3:][:-1],
                          doc.quiz_file + '.html')[0]
